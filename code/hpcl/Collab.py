...
# ###################### DATA PREPROCESSING ######################
# Load data
url = 'https://raw.githubusercontent.com/HPCL/ideas-uo/master/data/spack_developer_code.csv'
network_data = requests.get(url).text.split('\n')
...
for edge in edges:
    ...
    source, target, value = edge.split(",")

    cy_edge = {'data': {'id': source+target, 'source': source, 'target': target}}
    cy_target = {"data": {"id": target, "label": target}, 'classes':'File'}
    cy_source = {"data": {"id": source, "label": source}, 'classes':'Person'}
...
default_elements = cy_nodes+cy_edges

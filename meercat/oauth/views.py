...
from oauth.utils import gh_authenticate, gl_authenticate
from oauth.forms import RegistrationForm
from dashboard.utilities import gmail_send_message, save_debug_event
...
] = "1"  # This is set so requests-oauthlib does not throw an error over HTTP
...
def gitlab_callback(request):
    ...
    try:
        token = gitlab.fetch_token(
            token_url, client_secret=gl_client_secret, code=request.GET.get("code")
        )
    except AccessDeniedError as ade:
        print(ade)
        messages.error(
            request,
            "You have denied application access. To log in, please authorize through GitHub.",
        )
        ...
        messages.error(
            request,
            "There was an error during GitLab authorization. Please contact the meercat team and try again later.",
        )
        return redirect("login")
    ...
    if autheitcation_success:
        ...
        messages.warning(
            request,
            "Your GitLab account has not been registered in our site. Please register to continue.",
        )
...
def github_callback(request):
    ...
    try:
        ...
        messages.error(
            request,
            "There was an error during GitHub authorization. Please contact the meercat team and try again later.",
        )
        return redirect("login")
    ...
    if autheitcation_success:
        ...
        messages.warning(
            request,
            "Your GitHub account has not been registered in our site. Please register to continue.",
        )
        print(gh_user)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if sent is None:
                ...
                return render(
                    request, "oauth/success.html", {"name": name, "email": email}
                )
...
@login_required
def gmail_token(request):
    return render(request, "oauth/gmail_token.html")
...
def gmail_callback(request):
    ...
    try:
        ...
        messages.success(request, "You have issued a new OAuth2 gmail token")
    except ValueError as ve:
        save_debug_event(str(ve), request=request)
        messages.error(
            request,
            "There was a problem obtaining the OAuth token. For more information consult the event logs.",
        )

    return redirect("gmail_token")
